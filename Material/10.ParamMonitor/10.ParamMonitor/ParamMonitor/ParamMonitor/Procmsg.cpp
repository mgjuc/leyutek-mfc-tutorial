...
int gECG1MaxValue = 0, gECG1MinValue = 0xffff;
int gECG2MaxValue = 0, gECG2MinValue = 0xffff;
...
//ECG
static void OnECGWave              ( UCHAR* msg );
static void OnECGLead              ( UCHAR* msg );
...
static void OnECG(UCHAR* msg)
{
	...
	case DAT_ECG_LEAD:
		...
		break;
	case DAT_ECG_HR:
		gECGParams.hr = MAKEWORD(*(msg + 2), *(msg + 1));  
		break;
	default:
		break;
	}
...
static void OnECGWave( UCHAR* msg )
{
	gECGParams.wave1 = MAKEWORD( *(msg + 1), *msg);
	gECGParams.wave2 = MAKEWORD(*(msg + 3), *(msg + 2));
	
	// 计算峰峰值
	if (gECG1MaxValue < gECGParams.wave1)
	{	
		gECG1MaxValue = gECGParams.wave1;
	}
	
	if (gECG1MinValue > gECGParams.wave1)
	{
		gECG1MinValue = gECGParams.wave1;
	}
	
	// 计算峰峰值
	if (gECG2MaxValue < gECGParams.wave2)
	{	
		gECG2MaxValue = gECGParams.wave2;
	}
	
	if (gECG2MinValue > gECGParams.wave2)
	{
		gECG2MinValue = gECGParams.wave2;
	}
	
	gWndP->PostMessage(WM_DRAW_ECG_WAVE, gECGParams.wave1, gECGParams.wave2);
}
...
{
	//导联连接信息
	gECGStatus.bLeadDetachedLL = ((*msg) & 0x01) ? true : false; //第1位
	gECGStatus.bLeadDetachedLA = ((*msg) & 0x02) ? true : false; //第2位
	gECGStatus.bLeadDetachedRA = ((*msg) & 0x04) ? true : false; //第3位
	gECGStatus.bLeadDetachedV  = ((*msg) & 0x08) ? true : false; //第4位
	
	//过载信息
	gECGStatus.bOverload1 = (*(msg + 1) & 0x01) ? true : false;  //第1位
	gECGStatus.bOverload2 = (*(msg + 1) & 0x02) ? true : false;  //第2位
}
...
void  InitMsgHandle(void)
{
	...
	RegisterMsgHandle(DAT_ECG, OnECG);
}
