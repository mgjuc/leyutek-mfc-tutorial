...
protected:
	...
	int  mRespWaveX;        //X坐标
	int  mRespWaveY;        //Y坐标
	RECT mRespRect;         //在该矩形区域内画图
	int  mRespRectWidth;    //矩形区域宽度
	int  mRespRectHeight;   //矩形区域高度
	CPen mRespDrawPen;      //Resp波形画笔
	CPen mRespErasePen;     //Resp擦除画笔
	HDC  mRespDC;           //设备描述表句柄

	//ECG
	int  mECG1WaveX;        //X坐标
	int  mECG1WaveY;        //Y坐标
	RECT mECG1Rect;         //在该矩形区域内画图
	int  mECG1RectWidth;    //矩形区域宽度
	int  mECG1RectHeight;   //矩形区域高度
	CPen mECG1DrawPen;      //ECG1波形画笔
	CPen mECG1ErasePen;     //ECG1擦除画笔  
	HDC  mECG1DC;           //设备描述表句柄

	int  mECG2WaveX;        //X坐标
	int  mECG2WaveY;        //Y坐标
	RECT mECG2Rect;         //在该矩形区域内画图
	int  mECG2RectWidth;    //矩形区域宽度
	int  mECG2RectHeight;   //矩形区域高度
	CPen mECG2DrawPen;      //ECG2波形画笔
	CPen mECG2ErasePen;     //ECG2擦除画笔  
	HDC  mECG2DC;           //设备描述表句柄
	...
	void RefreshECGLeadStatus(void);
	void InitECGLeadStatus(void);
...
public:
	...
	afx_msg LRESULT DrawRespWave(WPARAM, LPARAM);
	afx_msg LRESULT DrawECGWave(WPARAM, LPARAM);
};
