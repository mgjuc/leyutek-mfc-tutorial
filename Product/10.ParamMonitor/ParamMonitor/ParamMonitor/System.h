...
extern StructRespParams gRespParams;
...
typedef struct  
{
	UCHAR leadSystem;     //导联选择(0:3-lead system, 1: 5-lead system)
}StructECGConfig;
...
typedef struct  
{
	bool  bLeadDetachedRA;   //导联RA是否脱落（脱落：true）
	bool  bLeadDetachedLA;   //导联LA是否脱落（脱落：true）
	bool  bLeadDetachedLL;   //导联LL是否脱落（脱落：true）
	bool  bLeadDetachedRL;   //导联RL是否脱落（脱落：true）
	bool  bLeadDetachedV;    //导联V是否脱落（脱落：true）
	
	bool  bOverload1;        //信号过载(true)
	bool  bOverload2;        //信号过载(true)
	bool  bCalibrating;      //是否正在校准，true：是；false：否
}StructECGStatus;
...
extern StructECGConfig gECGConfig;
extern StructECGStatus gECGStatus;
extern StructECGParams gECGParams;
extern const COLORREF  gColorOnOff[];
...
void PackToStr(PacketFrame Pack, int PackLen, CString& str);
