...
* 注    意:
*********************************************************************************************************/
void CPackUnpack::WriteUARTData()
{
    if (gSaveFlag)
    {
        if (mWirteFile)
        ...
        //mWirteFile.open(gSaveDir, std::ios::app); // 在文件末尾追加
        mWirteFile.open(gSaveDir, std::ios::ate | std::ios::out);
        gSaveFlag = false;
    }
    else
    {
        CString strValue;
        ::PackToStr(mCurrPack, mCurrPackLen, strValue);
        mWirteFile << strValue;
    }
}
